...
export class WeightComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.authService.weightGet().subscribe(
      data => {
  ...
  onSubmit(): void {
    ...
    this.authService.weightPost(weight).subscribe(
      data => {
  ...
  private buildLine() {
    ...
    var high = this.graphData.weight[0];

    for (var i = 0; i < this.graphData.weight.length; i++) {
      if (this.graphData.weight[i] > high) {
        high = this.graphData.weight[i];
      }
    }
    ...
    for (var i = 0; i < this.graphData.time.length; i++) {
      ...
      localData[i].weight = this.graphData.weight[i];
    }
    ...
    g.append("g")
      ...
      .attr("cy", (data: any) => yScale(data.weight))
      .attr("r", 5)
      .attr("fill", "#69b3a2")
